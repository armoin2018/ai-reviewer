...
export const performanceMonitor = (req, res, next) => {
    ...
    if (metrics.connections.active > SLA_THRESHOLDS.CONNECTIONS_ERROR) {
        ...
        error.code = ErrorCode.GITHUB_RATE_LIMIT;
        error.status = 503;
        return next(error);
    }
    res.on('finish', () => {
 ...
 */
function checkPerformanceThresholds(endpoint, responseTime, statusCode) {
...
export const getPerformanceMetrics = (req, res) => {
    ...
    Object.keys(processedMetrics.responseTime).forEach(endpoint => {
        ...
        if (p95Samples.length > 0) {
            ...
            processedMetrics.responseTime[endpoint].p95Value = p95Samples[p95Index] || 0;
        }
        // Remove raw p95 array from response
        delete processedMetrics.responseTime[endpoint].p95;
    });
    res.json({
 ...
 */
export const getHealthWithPerformance = (req, res) => {
    updateMemoryMetrics();
 ...
 */
export const generateLogSummary = (req, res) => {
    updateMemoryMetrics();
    const summary = {
        timestamp: new Date().toISOString(),
        period: {
