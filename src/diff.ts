/**
 * File information extracted from diff
 */
...
/**
 * Hunk information within a diff file
 */
 ...
 */
export interface DiffProcessingOptions {
  stripLevel?: number;
  maxFileSize?: number;
  allowBinary?: boolean;
 ...
 */
export function normalizeUnifiedDiff(diff: string, options: DiffProcessingOptions = {}): NormalizedDiff {
  const {
    stripLevel = 0,
 ...
 */
function detectDiffFormat(diff: string): 'git' | 'unified' | 'unknown' {
  const lines = diff.split('\n').slice(0, 10); // Check first 10 lines
  
  // Check for git diff format indicators
  const hasGitHeader = lines.some(line => 
  ...
  const hasUnifiedHeader = lines.some(line => 
    line.startsWith('--- ') || 
    line.startsWith('+++ ') ||
    line.match(/^@@ -\d+(?:,\d+)? \+\d+(?:,\d+)? @@/)
  );
 ...
 */
function parseGitDiff(diff: string, result: NormalizedDiff, stripLevel: number, allowBinary: boolean): void {
  const lines = diff.split('\n');
 ...
 */
function parseUnifiedDiff(diff: string, result: NormalizedDiff, stripLevel: number, allowBinary: boolean): void {
  const lines = diff.split('\n');
  let currentFile: Partial<DiffFile> | null = null;
  let currentHunk: Partial<DiffHunk> | null = null;
  let oldLineNum = 0;
 ...
 */
function applyStripLevel(path: string, stripLevel: number): string {
  if (stripLevel === 0) return path;
 ...
 */
function finalizeFile(file: Partial<DiffFile>, result: NormalizedDiff): void {
  if (!file.path) {
    result.errors.push('File found without valid path');
    return;
  }
  
  result.files.push(file as DiffFile);
}

/**
 * Extract just the file paths from a diff (legacy compatibility)
 */
export function extractDiffFiles(diff: string, stripLevel = 0): string[] {
 ...
 */
export function validateDiff(diff: string): { isValid: boolean; errors: string[] } {
