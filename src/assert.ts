...
type Finding = { id: string; status: "pass" | "fail" | "warn" | "na"; note: string; file?: string; line?: number };
...
export function assertCompliance(params: {
  ...
  if (secretsDetected > 0) genericFindings.push({ level: "fail", note: `Potential secrets detected: ${secretsDetected}` });

  if (!diff.endsWith("\n")) genericFindings.push({ level: "warn", note: "Diff does not end with a newline (EOF newline recommended)." });

  const largeFiles: string[] = [];

  if (requireTests && codeFiles.length > 0 && testFiles.length === 0) {
    genericFindings.push({ level: "warn", note: "Code changed but no test files changed. Consider adding/updating tests." });
  }
  ...
  if (directives.disallowPath.length) {
    ...
    for (const p of directives.disallowPath) {
      ...
      if (violated.length) genericFindings.push({ level: "fail", note: `disallow-path violated: ${p} matched ${violated.join(", ")}` });
    }
  }
  if (directives.forbidPattern.length) {
    ...
    for (const p of directives.forbidPattern) {
      ...
      if (hits > 0) genericFindings.push({ level: "fail", note: `forbid-pattern hit ${hits} occurrences for ${p}` });
    }
  }
  if (directives.requireFile.length) {
    ...
    for (const p of directives.requireFile) {
      ...
      if (!present) genericFindings.push({ level: "fail", note: `require-file not satisfied: expected a changed file matching ${p}` });
    }
  }
  if (directives.requireTests !== null) {
    ...
    if (directives.requireTests && codeFiles.length > 0 && testFiles.length === 0) {
      genericFindings.push({ level: "fail", note: "Directive require-tests=true violated: no test files changed." });
    }
  }
  ...
  if (directives.denyImport.length) {
    ...
    for (const [file, lines] of numbered.entries()) {
      ...
      for (const ln of lines) for (const p of directives.denyImport) if (p.test(ln.text)) {
        genericFindings.push({ level: "fail", note: `deny-import violated for ${file}: "${ln.text.trim()}" matches ${p}` });
      }
    }
  }
  ...
  if (directives.enforceLicenseHeader.length) {
    ...
    for (const [file, hunks] of perFileHunks.entries()) {
      ...
      for (const p of directives.enforceLicenseHeader) {
        if (!p.test(first)) genericFindings.push({ level: "fail", note: `enforce-license-header failed for ${file}: header regex ${p} not found in first added hunk` });
      }
    }
  }
  ...
  if (fileContents) {
    if (directives.enforceLicenseHeaderExact.length) {
      ...
      for (const [file, content] of Object.entries(fileContents)) {
        ...
        for (const p of directives.enforceLicenseHeaderExact) {
          if (!p.test(head)) genericFindings.push({ level: "fail", note: `enforce-license-header-exact failed for ${file}: regex ${p} not found at file start` });
        }
      }
    }
    if (directives.denyHeader.length) {
      ...
      for (const [file, content] of Object.entries(fileContents)) {
        if (!CODE_RE.test(file)) continue;
        const head = content.slice(0, 2000);
        for (const p of directives.denyHeader) if (p.test(head)) {
          genericFindings.push({ level: "fail", note: `deny-header violated for ${file}: regex ${p} matched file header` });
        }
      }
    }
    if (Object.keys(directives.enforceLicenseHeaderLang).length || directives.enforceLicenseHeaderCanonical) {
      ...
      for (const [file, content] of Object.entries(fileContents)) {
        ...
        if (!langRe) continue;
        const head = content.slice(0, 2000);
        if (!langRe.test(head)) genericFindings.push({ level: "fail", note: `enforce-license-header-lang failed for ${file} (lang=${lang}): regex ${langRe} not found at file start` });
      }
    }
  }

  const findings = (checklist || []).map(r => ({ id: r.id, status: "na", note: `Cannot deterministically verify: "${truncate(r.text,120)}". Ensure compliance.` }));
...
function parseChangedFiles(diff: string): string[] {
  ...
  for (const l of lines) {
    ...
    if (m2) { files.add(m2[1]); files.add(m2[2]); }
  }
...
function collectAddedLinesWithNumbersByFile(diff: string): Map<string, Array<{line:number,text:string}>> {
  const map = new Map<string, Array<{line:number,text:string}>>();
  let currentFile: string | null = null;
  let newLine = 0;
  const lines = diff.replace(/\r\n/g, "\n").split("\n");
  for (const l of lines) {
    if (l.startsWith("diff --git ")) { currentFile = null; continue; }
    const mHeader = l.match(/^\+\+\+\s+[ab]\/(.+)$/);
    if (mHeader) { currentFile = mHeader[1]; if (!map.has(currentFile)) map.set(currentFile, []); continue; }
    const mHunk = l.match(/^@@\s+-\d+(?:,\d+)?\s+\+(\d+)(?:,\d+)?\s+@@/);
    if (mHunk) { newLine = parseInt(mHunk[1] || "0", 10); continue; }
    if (!currentFile) continue;
    if (l.startsWith("+") && !l.startsWith("+++")) { map.get(currentFile)!.push({ line: newLine, text: l.slice(1) }); newLine++; }
    else if (l.startsWith(" ") || l === "") { newLine++; }
  }
  return map;
}
...
function parseDirectives(md: string): Directives {
  ...
  const res: Directives = {
    disallowPath: [], forbidPattern: [], requireFile: [], requireTests: null, maxFileBytes: null,
    requireLabel: [], blockCommitType: [], enforceLicenseHeader: [], denyImport: [], enforceLicenseHeaderExact: [],
    denyHeader: [], enforceLicenseHeaderLang: {}, enforceLicenseHeaderCanonical: null
  };
  for (const raw of lines) {
    const m = raw.match(/^\s*\[ASSERT\]\s*(.+)$/i); if (!m) continue;
    const stmt = m[1].trim(); const parts = stmt.split(/:\s*/); const key = (parts.shift() || "").trim().toLowerCase(); const val = parts.join(":").trim();
    try {
      switch (key) {
        case "disallow-path": res.disallowPath.push(new RegExp(val)); break;
        case "forbid-pattern": res.forbidPattern.push(new RegExp(val)); break;
        case "require-file": res.requireFile.push(new RegExp(val)); break;
        case "require-tests": res.requireTests = /true|1|yes/i.test(val); break;
        case "max-file-bytes": res.maxFileBytes = Number(val) || null; break;
        case "require-label": res.requireLabel.push(new RegExp(val)); break;
        case "block-commit-type": res.blockCommitType.push(new RegExp(val)); break;
        case "enforce-license-header": res.enforceLicenseHeader.push(new RegExp(val)); break;
        case "deny-import": res.denyImport.push(new RegExp(val)); break;
        case "enforce-license-header-exact": res.enforceLicenseHeaderExact.push(new RegExp(val)); break;
        case "deny-header": res.denyHeader.push(new RegExp(val)); break;
        case "enforce-license-header-lang":
          const eq = val.indexOf("="); if (eq > 0) { const lang = val.slice(0,eq).trim().toLowerCase(); const pat = val.slice(eq+1).trim(); res.enforceLicenseHeaderLang[lang] = new RegExp(pat); }
          break;
        case "enforce-license-header-canonical": res.enforceLicenseHeaderCanonical = new RegExp(val); break;
        default: break;
      }
    } catch {}
  }
