...
exports.getDefaultSecretPatterns = getDefaultSecretPatterns;
exports.validateSecretPattern = validateSecretPattern;
/**
 ...
 */
function isWhitelisted(content, whitelistPatterns) {
    return whitelistPatterns.some(pattern => pattern.test(content));
}
/**
 * Extract context around a match for better reporting
 */
function extractContext(content, matchIndex, matchLength, contextLines = 2) {
...
function scanForSecrets(content, filePath, options = {}) {
    const { patterns = DEFAULT_SECRET_PATTERNS, whitelistPatterns = [], enableEntropyAnalysis = true, minEntropyThreshold = 4.0, maxFileSize = 1024 * 1024, // 1MB
    allowedFileTypes = ['.ts', '.js', '.py', '.java', '.go', '.rb', '.php', '.cs', '.cpp', '.c', '.h'], excludeFiles = [/node_modules/, /\.git/, /coverage/, /dist/, /build/] } = options;
    const findings = [];
 ...
 */
function scanDiffForSecrets(diffContent, options = {}) {
    const startTime = Date.now();
    const findings = [];
    const errors = [];
    try {
 ...
 */
function validateSecretPattern(pattern) {
    const errors = [];
