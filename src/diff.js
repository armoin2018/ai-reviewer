...
exports.normalizeUnifiedDiff = normalizeUnifiedDiff;
exports.extractDiffFiles = extractDiffFiles;
 ...
 */
function normalizeUnifiedDiff(diff, options = {}) {
...
function parseUnifiedDiff(diff, result, stripLevel, allowBinary) {
    ...
    for (let i = 0; i < lines.length; i++) {
        ...
        if (line.startsWith('--- ')) {
            ...
            if (pathMatch && pathMatch[1] !== '/dev/null') {
                currentFile.oldPath = applyStripLevel(pathMatch[1], stripLevel);
 ...
 */
function extractDiffFiles(diff, stripLevel = 0) {
