...
async function installationToken(owner: string): Promise<string> {
  ...
  const installation = Array.isArray(inst) ? inst.find((i: any) => i.account?.login?.toLowerCase() === owner.toLowerCase()) : undefined;
  if (!installation) throw new Error(`No installation found for owner ${owner}`);
  const tokenResp = await fetch(installation.access_tokens_url, { method: "POST", headers: { Authorization: `Bearer ${token}`, Accept: "application/vnd.github+json" } });
...
export async function getFileContents(params: { owner: string; repo: string; ref: string; paths: string[] }): Promise<Record<string,string>> {
  ...
  for (const p of paths) {
    ...
    const resp = await fetch(url, { headers: { Authorization: `Bearer ${token}`, Accept: "application/vnd.github.raw" } });
    if (resp.ok) out[p] = await resp.text();
  }
  return out;
}

export async function loadRules({ owner, repo, ref }: { owner: string; repo: string; ref: string; }): Promise<{
  copilot: string | null;
  ...
  async function listDir(path: string): Promise<Array<{path:string,type:string,name:string}>> {
    const url = `https://api.github.com/repos/${owner}/${repo}/contents/${encodeURIComponent(path)}?ref=${encodeURIComponent(ref)}`;
    const data: any = await getJson(url, token);
  ...
  try {
    ...
    for (const e of ins) if (e.type === "file" && /\.(md|mdx|txt|ya?ml|json)$/i.test(e.name)) {
      const c = await getContent(e.path); if (c) { instructions[e.name] = c; files.push(e.path); }
    }
  } catch {}
  try {
    const per = await listDir(".github/personas");
    for (const e of per) if (e.type === "file" && /\.(md|mdx|txt|ya?ml|json)$/i.test(e.name)) {
      const c = await getContent(e.path); if (c) { personas[e.name] = c; files.push(e.path); }
    }
  } catch {}
