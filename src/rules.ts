/**
 * Rule representation with priority and category
 */
...
/**
 * Rule extraction patterns for different markdown formats
 */
 ...
 */
const PRIORITY_WEIGHTS: Record<Rule['priority'], number> = {
  critical: 4,
  high: 3,
  medium: 2,
  low: 1
 ...
 */
function extractRules(markdown: string): Rule[] {
  ...
  let currentContext = {
    section: '',
    category: 'compliance' as Rule['category']
  };
...
function sortRules(rules: Rule[]): Rule[] {
  return rules.sort((a, b) => {
    ...
    const categoryOrder: Rule['category'][] = [
 ...
 */
function getCacheKey(markdown: string, maxItems: number): string {
 ...
 */
export function summarizeRules(markdown: string, maxItems = 400): Rule[] {
  // Check cache first
 ...
 */
export function getRuleStatistics(rules: Rule[]): {
  total: number;
  byPriority: Record<Rule['priority'], number>;
  byCategory: Record<Rule['category'], number>;
} {
  const byPriority: Record<Rule['priority'], number> = {
    critical: 0, high: 0, medium: 0, low: 0
  };
  
  const byCategory: Record<Rule['category'], number> = {
