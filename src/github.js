...
exports.getGitHubCacheStats = getGitHubCacheStats;
...
async function appJwt() {
    ...
    return jsonwebtoken_1.default.sign({ iat: now - 60, exp: now + 9 * 60, iss: APP_ID }, PRIVATE_KEY, {
        algorithm: 'RS256',
    });
}
async function installationToken(owner) {
    const token = await appJwt();
    const instResp = await (0, node_fetch_1.default)(`https://api.github.com/app/installations`, {
    ...
    const tokenResp = await (0, node_fetch_1.default)(installation.access_tokens_url, {
        method: 'POST',
        headers: { Authorization: `Bearer ${token}`, Accept: 'application/vnd.github+json' },
    });
    if (!tokenResp.ok)
...
async function loadRules({ owner, repo, ref, }) {
    const token = await installationToken(owner);
